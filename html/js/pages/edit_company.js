...
// CONTINUE FROM HERE
	...
	var	RenderCompanyOpenVacancies = function()
	{
		...
		companyProfile.open_vacancies.forEach(function(item, i, arr)
		{
			...
			{
				var		publishDaysRemain = Math.floor((publishFinishTS.getTime() - currentTS.getTime()) / (24 * 3600 * 1000) );
				...
				spanStatus = $("<span>").addClass("fa-stack")
										...
										.attr("data-toggle", "tooltip")
										.attr("data-placement", "top")
										.attr("title", "Опубликована (еще " + publishDaysRemain + " " + system_calls.GetDaysSpelling(publishDaysRemain) + ")");
			}
			else
			{
				spanStatus = $("<span>").addClass("fa-stack")
